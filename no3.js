...
rl.question("Masukkan kalimat: ", (inputValue) => {
  const converted = inputValue.split("").map((value) => conversion[value]);
  ...
  const finalOutput = breakdown.map((digit) => digitToLetter[digit]).join("");
  console.log(`Hasil konversi: ${finalOutput}`);
  rl.close();
});
